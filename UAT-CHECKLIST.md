...
| ID | Preconditions | Steps | Expected | Layer | Covers |
|---|---|---|---|---|---|
| ultimail.attachment.gui-guard | `language: en`; sender holds `ultimail.admin.multiattach`; `AttachmentSelectorPage` open (see `ultimail.sendmail.attach.admin-multi`) | Place ONE item in the content area (slots 0-44), confirm it is NOT cancelled; then completely fill all 45 content-area slots (so `topInventory.firstEmpty()` returns no slot inside the content area at all); then attempt a shift-click of an item from the player's own inventory (which, with the content area full, can now only target the toolbar, slots 45+); then close the GUI via ESC without clicking Confirm | The single-item content-area placement succeeds normally; once the content area is completely full, the shift-click is cancelled (the item stays in the player's inventory, not the toolbar); closing without confirming returns every item placed in the content area back to the player's inventory | pixel | AttachmentSelectorPage |
