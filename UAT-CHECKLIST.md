...
| ID | Preconditions | Steps | Expected | Layer | Covers |
|---|---|---|---|---|---|
| ultimail.attachment.gui-guard | `language: en`; sender holds `ultimail.admin.multiattach`; `AttachmentSelectorPage` open (see `ultimail.sendmail.attach.admin-multi`) | Place ONE item in the content area (slots 0-44), confirm it is NOT cancelled; then close the GUI via ESC without clicking Confirm | The single-item content-area placement succeeds normally (a shift-click INTO the content area while it has room is deliberately allowed, not guarded); closing without confirming returns every item placed in the content area back to the player's inventory. This class's own toolbar-protection guard (`onInventoryClick`'s `targetSlot >= contentSize` check) is source-confirmed unreachable — `setupBottomToolbar()` pre-fills all 9 toolbar slots before any content is placed, so `firstEmpty()` can only ever return a content-area index or `-1`, never a toolbar index; see `UltiKits/UltiMail#26` | pixel | AttachmentSelectorPage |
