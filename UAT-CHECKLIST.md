...
| ID | Preconditions | Steps | Expected | Layer | Covers |
|---|---|---|---|---|---|
| ultimail.config.mail-yml | Fresh `config/mail.yml` at its shipped default | Load the file; confirm all 23 keys listed under `FEATURES.md`'s `## Configuration` section are present at their documented defaults; then set `max-items: 2` (default 27) and, via `/mail sendall <content> items` (the path that reads this key — see `ultimail.gui.attachment-selector.neg-too-many`), attempt to attach 3 items; separately set `send-cooldown: 20` (default 10, NOT 120 — a value must fall strictly between the old and new limits to actually distinguish them) and, after a successful `/sendmail` send (content submitted), wait 12 seconds (past the OLD 10s limit but inside the NEW 20s one) before submitting a second send's content; separately set `notify-on-join: false` (default true) and confirm no join notification fires for a player with unread mail. Do NOT vary `mail-expire-days`, `messages.new-mail`, or `messages.mail-sent` expecting an observable effect — none has one (`UltiKits/UltiMail#23`) | All 23 keys present at their documented defaults before any change; (a) attaching 3 items with `max-items: 2` triggers `send_items_too_many` where it previously would not have at 27; (b) the second send's content, submitted 12 seconds after the first (past the old 10s cooldown but inside the new 20s one), is STILL refused with `send_cooldown` — proving the new, longer cooldown took effect, not merely that the old one expired; (c) no join notification fires with `notify-on-join: false`, where it previously would have | server | |
