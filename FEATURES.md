...
"ultimail.use", description = "command_description_mail")` (a language-file key: `Mail system`
under `language: en`), `@CmdTarget(PLAYER)`. Twelve `@CmdMapping` sites, all listed below.
...
| ultimail.mail.claim | Claim the item attachments of a specific inbox mail by its listed index, refusing if already claimed, has no items, or the player's inventory lacks enough empty slots. The claim is recorded before the items are handed over; a claim the storage cannot record is refused with `Your claim could not be recorded, so nothing was given. Please try again.` (red), nothing is given, and the mail stays claimable. On the JSON storage backend a write reaches only an in-memory cache flushed to disk on a timer, so a disk failure there is not seen at claim time | command | `/mail claim <index>` | ultimail.use | player | player | brief | MailCommand#claim |
...
| ultimail.mail.help | Print `/mail` usage, one line per sub-command including `sentgui` (UltiKits/UltiMail#21), each line the language file's `help_*` text with the command part coloured and written once; the `sendall` line is shown only to a sender holding `ultimail.admin.sendall` | command | bare `/mail` | ultimail.use | player | player | none | MailCommand#handleHelp |
...
| ultimail.mail.read-detail | Open a specific inbox mail's full detail view by its listed index: sender, subject, timestamp, content, and (if applicable) claim-hint or already-claimed status; marks the mail read and executes any attached commands that have not run yet, at most once: the executed marker is written before any command runs, and when it cannot be written none runs and the reader is told `This mail's attached commands could not be recorded as run, so none of them ran. Read the mail again to retry.` (red); a command that throws, or that the server reports as not run (an unknown command, or one whose handler reported failure), is logged at WARNING naming the mail and the command, is not run again, and does not stop the commands after it | command | `/mail read <index>` | ultimail.use | player | player | brief | MailCommand#readByIndex |
...
"ultimail.recall", description = "command_description_recall")` (a language-file key: `Invite
players back to the server` under `language: en`), no class-level `@CmdTarget` (defaults to accepting a plain `CommandSender`). Two
...
| ID | Feature | Kind | How to reach | Permission | Target | Tier | Manual | Source |
|---|---|---|---|---|---|---|---|---|
| ultimail.recall.help | Print `/recall` usage from the language file (`help_recall_title`, `help_recall_default`, `help_recall_custom`, `arg_recall_message`; UltiKits/UltiMail#22) | command | literal `/recall help` (bare `/recall` reaches `sendRecall` instead, see below) | ultimail.recall | both | admin | none | RecallCommand#handleHelp |
...
"ultimail.send", description = "command_description_sendmail")` (a language-file key: `Send mail`
under `language: en`), `@CmdTarget(PLAYER)`. Three `@CmdMapping` sites.
...
| ultimail.integration.game-mail-service | Framework-pluggable `GameMailService` implementation (priority 100) letting another module send game mail, send system mail, or query unread count through `MailService` without depending on this module's command classes directly; its own `notifyNewMail(Player)` sends the language file's `notify_unread_mail` with the unread count, in the server's language (UltiKits/UltiMail#21) | event | any other module or the framework calling a held `GameMailService` reference | n/a | n/a | internal | brief | UltiMailGameMailService#sendMail, UltiMailGameMailService#notifyNewMail |
| ultimail.notify.on-join | On join, if `notify-on-join` and the player has unread mail, send a delayed clickable chat notification: the language file's `notify_new_mail` with the unread count in its `{0}`, then `notify_click_to_view` (which carries its own brackets) as the clickable part, click-to-run `/mail read`, hover text `notify_hover_text`; the `&` colour codes of both are shown as colour (UltiKits/UltiMail#24) | event | join the server with at least one unread mail, `notify-on-join: true` (shipped default) | n/a | n/a | player | detailed | MailNotifyListener#onPlayerJoin |
...
| ultimail.gui.mailbox | Paginated inbox: one icon per mail (book if read, writable book if unread), lore shows sender/time/content preview and claim state; clicking marks read, executes attached commands that have not run yet (at most once, as `ultimail.mail.read-detail` describes), and claims items if space allows — a claim the storage cannot record is answered with `Your claim could not be recorded, so nothing was given. Please try again.` and nothing is given | gui | `ultimail.mail.read-gui` | n/a | n/a | player | detailed | MailboxGUI#handleMailClick |
...
`UltiKits/UltiMail#23`, which repeats the removed-key warning — see
`ultimail.lifecycle.removed-key-warning` — and, like `registerSelf()`, writes the text settings of
`config/mail.yml` in the server's language — see `ultimail.lifecycle.config-text-language`. `/ul reload UltiMail` (and `/ul reload`, which reloads
...
| ID | Feature | Kind | How to reach | Permission | Target | Tier | Manual | Source |
|---|---|---|---|---|---|---|---|---|
| ultimail.lifecycle.config-text-language | On module enable and again on every reload of it (`UltiMail#registerSelf`, `#onReload`, both after the framework has read `config/mail.yml` and loaded the language — never from a configuration change listener, which the framework fires before it reloads the language): each of the six text settings (`messages.mail-received`, `recall.server-name`, `recall.subject`, `recall.content`, `email.recall-subject`, `email.recall-content`) that is still built-in text — the Chinese default an earlier version shipped, or this jar's English or Chinese text for it (read from the module jar, never from the language files on disk) — and differs from the current text is replaced with the text in the server's language, and the file is saved once. An untouched value therefore follows a `language` switch in both directions; a value that differs in any way, even by one character — such as a `recall.server-name` set to the server's own name — is the operator's and is kept byte for byte; a text that would break the setting's `@NotEmpty` is never written; a second start with the same language writes nothing. A single-module `/ul reload UltiMail` does not re-read the framework's `language`, so a changed `language` is picked up on a bare `/ul reload` or a restart. A failed save is logged as one warning from the language file (`log_config_default_save_failed`), and the module keeps using the new text for that session. The text written is this jar's own built-in text for the server's language (the jar's `lang/<language>.yml`), not the extracted language file on disk, so every value the module writes is one it recognises again; these settings are edited in the config file, and an edit of the extracted language file does not change them (earlier versions never read them from the language file either; the text source decision of 2026-09-25) | event | automatic, at module enable and at `/ul reload` | n/a | n/a | admin | brief | UltiMail#registerSelf, UltiMail#onReload, MailConfig#materializeText, ConfigTextDefaults |
| ultimail.lifecycle.reload | `/ul reload UltiMail` re-reads `config/mail.yml` into the running module, so an edited value such as `max-subject-length` applies to the next mail submitted without a restart; this module's own reload work is the removed-key warning (`ultimail.lifecycle.removed-key-warning`) and writing its text settings in the server's language (`ultimail.lifecycle.config-text-language`), and it prints no reload line of its own. Before `UltiKits/UltiMail#20` the module's reload override replaced the framework's reload and only logged, so an edit took effect only after a restart | event | `/ul reload UltiMail` (framework calls `reloadSelf()`, which reloads configuration, refreshes language, reports `@ConditionalOnConfig` drift and logs its own per-module line) | n/a | n/a | admin | brief | MailService#sendMail |
| ultimail.lifecycle.removed-key-warning | When this module loads, and on every reload of it, it checks its operator's own `config/mail.yml` for the three keys `UltiKits/UltiMail#23` removed (`mail-expire-days`, `messages.new-mail`, `messages.mail-sent`), which the framework never deletes from an existing file, and logs one WARNING per key still present, naming the file and the key and saying where the setting went: mail expiry is a feature request (`UltiKits/UltiMail#34`), and each message's text is edited in this module's language file (`notify_new_mail`, `mail_sent_success`), whose placeholders are `{0}` and `{RECEIVER}` where the removed keys used `{COUNT}` and `{PLAYER}` (the join notification does not yet put the count into `{0}`, `UltiKits/UltiMail#24`). A file without those keys produces no line; a missing or unparseable file produces none either; a failure inside the check is logged and never fails the load or the reload | event | server start (the framework calls `registerSelf()` when it loads this module), and `/ul reload UltiMail` or `/ul reload` (framework calls `reloadSelf()`, which calls this module's `onReload()` last) | n/a | n/a | admin | brief | UltiMail#registerSelf, UltiMail#onReload, RemovedConfigKeys#warnAboutLeftovers |
...
| ultimail.config.mail.email.recall-content | Real-email recall body template; `{SERVER}`/`{PLAYER}`/`{SENDER}` placeholders | config | `config/mail.yml: email.recall-content (default: written in the server's language when the module starts — under `language: en` `Dear {PLAYER},\n\nThe {SERVER} server misses you! Come back and take a look, we look forward to seeing you again!\n\nSender: {SENDER}`, under `language: zh` the Chinese text every earlier version shipped; a value that is still built-in text follows `language`, an edited one is kept, see `ultimail.lifecycle.config-text-language`)` | n/a | n/a | admin | none | MailConfig#getRecallEmailContent, RecallCommand#sendRealEmail |
| ultimail.config.mail.email.recall-subject | Real-email recall subject template; `{SERVER}` placeholder | config | `config/mail.yml: email.recall-subject (default: written in the server's language when the module starts — under `language: en` `[{SERVER}] We miss you!`, under `language: zh` the Chinese text every earlier version shipped; a value that is still built-in text follows `language`, an edited one is kept, see `ultimail.lifecycle.config-text-language`)` | n/a | n/a | admin | none | MailConfig#getRecallEmailSubject, RecallCommand#sendRealEmail |
...
| ultimail.config.mail.messages.mail-received | Chat message shown to an ONLINE receiver the instant a mail arrives; `{SENDER}` substituted. This file's only `messages.*` key: its two former siblings, `messages.new-mail` and `messages.mail-sent`, were never read and were removed by UltiKits/UltiMail#23 | config | `config/mail.yml: messages.mail-received (default: written in the server's language when the module starts — under `language: en` `&e[Mail] &fYou received a new mail from &a{SENDER}&f!`, under `language: zh` the Chinese text every earlier version shipped; a value that is still built-in text follows `language`, an edited one is kept, see `ultimail.lifecycle.config-text-language`)` | n/a | n/a | admin | brief | MailConfig#getMailReceivedMessage, MailService#notifyReceiver |
...
| ultimail.config.mail.recall.content | In-game recall mail body template; `{SERVER}`/`{SENDER}` placeholders (overridden entirely if `/recall <message>`'s custom message argument is supplied) | config | `config/mail.yml: recall.content (default: written in the server's language when the module starts — under `language: en` `Dear player, {SERVER} misses you!\n\nCome back and take a look, we look forward to seeing you again!\n\nSender: {SENDER}`, under `language: zh` the Chinese text every earlier version shipped; a value that is still built-in text follows `language`, an edited one is kept, see `ultimail.lifecycle.config-text-language`)` | n/a | n/a | admin | brief | MailConfig#getRecallContent, RecallCommand#sendGameMail |
| ultimail.config.mail.recall.server-name | Server display name substituted into recall mail/email subject and content, and used as the in-game mail's sender name — NOT shown in the admin's own recall-summary lines, which contain only fixed text and numeric counts and never read this key | config | `config/mail.yml: recall.server-name (default: written in the server's language when the module starts — under `language: en` `Minecraft Server`, under `language: zh` the Chinese text every earlier version shipped; a value that is still built-in text follows `language`, an edited one is kept, see `ultimail.lifecycle.config-text-language`)` | n/a | n/a | admin | brief | MailConfig#getServerName, RecallCommand#sendGameMail, RecallCommand#sendRealEmail |
| ultimail.config.mail.recall.subject | In-game recall mail subject template; `{SERVER}` placeholder | config | `config/mail.yml: recall.subject (default: written in the server's language when the module starts — under `language: en` `[{SERVER}] Come back to us`, under `language: zh` the Chinese text every earlier version shipped; a value that is still built-in text follows `language`, an edited one is kept, see `ultimail.lifecycle.config-text-language`)` | n/a | n/a | admin | brief | MailConfig#getRecallSubject, RecallCommand#sendGameMail |
