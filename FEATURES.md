...
| ultimail.config.mail.email.recall-content | Real-email recall body template; `{SERVER}`/`{PLAYER}`/`{SENDER}` placeholders | config | `config/mail.yml: email.recall-content (blank; a blank value shows the language file's `recall_email_content` in the server's language, and a value exactly equal to the Chinese default every earlier version shipped is blanked on start-up and reload, see `ultimail.config.mail.legacy-text-defaults`)` | n/a | n/a | admin | none | MailConfig#getRecallEmailContent, RecallCommand#sendRealEmail |
| ultimail.config.mail.email.recall-subject | Real-email recall subject template; `{SERVER}` placeholder | config | `config/mail.yml: email.recall-subject (blank; a blank value shows the language file's `recall_email_subject` in the server's language, and a value exactly equal to the Chinese default every earlier version shipped is blanked on start-up and reload, see `ultimail.config.mail.legacy-text-defaults`)` | n/a | n/a | admin | none | MailConfig#getRecallEmailSubject, RecallCommand#sendRealEmail |
...
| ultimail.config.mail.legacy-text-defaults | On module enable and on every reload, each of the five text settings (`messages.mail-received`, `recall.subject`, `recall.content`, `email.recall-subject`, `email.recall-content`) whose value is BYTE-IDENTICAL to the Chinese default every earlier version shipped is blanked and the file saved, so the language file's text is shown in the server's language; any other value, including an operator's own, is kept as written. Idempotent — a blank value matches nothing (maintainer ruling 2026-09-24 (d)) | config | automatic on module enable/reload, for a `config/mail.yml` still holding an old shipped default | n/a | n/a | internal | brief | MailConfig#migrateLegacyDefaults, UltiMail#registerSelf, UltiMail#onReload |
...
| ultimail.config.mail.messages.mail-received | Chat message shown to an ONLINE receiver the instant a mail arrives; `{SENDER}` substituted (the language file's `notify_mail_received`, shown while the setting is blank, names the sender `{0}`, also substituted). This file's only `messages.*` key: its two former siblings, `messages.new-mail` and `messages.mail-sent`, were never read and were removed by UltiKits/UltiMail#23 | config | `config/mail.yml: messages.mail-received (blank; a blank value shows the language file's `notify_mail_received` in the server's language, and a value exactly equal to the Chinese default every earlier version shipped is blanked on start-up and reload, see `ultimail.config.mail.legacy-text-defaults`)` | n/a | n/a | admin | brief | MailConfig#getMailReceivedMessage, MailService#notifyReceiver |
...
| ultimail.config.mail.recall.content | In-game recall mail body template; `{SERVER}`/`{SENDER}` placeholders (overridden entirely if `/recall <message>`'s custom message argument is supplied) | config | `config/mail.yml: recall.content (blank; a blank value shows the language file's `recall_content` in the server's language, and a value exactly equal to the Chinese default every earlier version shipped is blanked on start-up and reload, see `ultimail.config.mail.legacy-text-defaults`)` | n/a | n/a | admin | brief | MailConfig#getRecallContent, RecallCommand#sendGameMail |
...
| ultimail.config.mail.recall.subject | In-game recall mail subject template; `{SERVER}` placeholder | config | `config/mail.yml: recall.subject (blank; a blank value shows the language file's `recall_subject` in the server's language, and a value exactly equal to the Chinese default every earlier version shipped is blanked on start-up and reload, see `ultimail.config.mail.legacy-text-defaults`)` | n/a | n/a | admin | brief | MailConfig#getRecallSubject, RecallCommand#sendGameMail |
