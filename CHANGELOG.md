...
- A mail's attachment and its attached commands are now recorded as claimed before they are handed
  ...
  mail again. An attached command that fails, or that the server does not run (an unknown command), is
...
- 修复：邮件附件与附带命令先写入已领取记录再发放，记录写失败时拒绝领取，不再可以重复领取。存储无法记录领取时，`/mail claim`
  ...
  并提示读者重新阅读邮件。执行失败或未被服务器执行（如未知命令）的附带命令会连同邮件和命令一起记入日志，不会再次执行，也不再阻止其后的命令。对无法读取的附件执行
